	...
	class MySQLDB
	{
		...
		*/
		function updateUserField($username, $field, $value){
            ...
            mysql_real_escape_string($username));
			return mysql_query($q, $this->connection);
			$logQ = sprintf("INSERT INTO ".TBL_LOG." VALUES (NULL,".$session->CWID.","."'Updated user field: ".$field." to value: ".$value."',NULL,'".$session->referrer."')");
			mysql_query($logQ, $this->connection);
		}
		...
		*/
        ...
        function addEvent2($title, $type, $course, $crn, $seats, $notes, $dateStart, $dateEnd, $room, $user, $series, $time, $conflict){
			...
			if ($conflict < 1){
				...
				$q2 = sprintf("INSERT INTO mail (UserTo, UserFrom, Subject, Message, SentDate, status) VALUES ('alyssa','admin','NOTICE! Conflict Created','$message','$date','unread')");
			...
			$logQ = sprintf("INSERT INTO ".TBL_LOG." VALUES (NULL,".$session->CWID.","."'Event created with no conflicts:".$title."',NULL,'".$session->referrer."')");
			mysql_query($logQ, $this->connection);
			if(!$result || (mysql_num_rows($result) < 1)){
				return NULL;
			}
		...
		function editEventB($title, $type, $seats, $notes, $dateStart, $dateEnd, $room, $conflict, $eventid){
			...
			$q = sprintf("Update ".TBL_EVENTS." set title = '$title', type = '$type', seats = $seats, room = $room, notes = $notes, dateStart = $dateStart, dateEnd = $dateEnd, ".time().", $approval  where event_id = $eventid");
			$myfile = fopen("error.txt", "a") or die(print_r($q));
			$result = mysql_query($q, $this->connection);
			$logQ = sprintf("INSERT INTO ".TBL_LOG." VALUES (NULL,".$session->CWID.","."'Event updated: ".$title."',NULL,'".$session->referrer."')");
			mysql_query($logQ, $this->connection);
			if(!$result || (mysql_num_rows($result) < 1)){
				return NULL;
			}
		...
		function editEventC($eventid, $notes){
			...
			//$myfile = fopen("error.txt", "a") or die(print_r($q));
			$result = mysql_query($q, $this->connection);
			$logQ = sprintf("INSERT INTO ".TBL_LOG." VALUES (NULL,".$session->CWID.","."'Event updated notes in event: ".$eventid."',NULL,'".$session->referrer."')");
			mysql_query($logQ, $this->connection);
			return TRUE;
		...
		function deleteEvent($eventid){
			...
			$result = mysql_query($q, $this->connection);
			$logQ = sprintf("INSERT INTO ".TBL_LOG." VALUES (NULL,".$session->CWID.","."'Event deleted: ".$eventid."',NULL,'".$session->referrer."')");
		mysql_query($logQ, $this->connection);
		if(!$result || (mysql_num_rows($result) < 1)){
			return NULL;
		}
